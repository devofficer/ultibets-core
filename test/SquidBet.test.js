...
describe("SquidBets", function () {
  ...
  beforeEach(async function () {
    ...
    const SquidBetThirdRound = await ethers.getContractFactory("SquidBetThirdRound");
    ...
    ///start round 
    this.squidBetStartRound = await SquidBetStartRound.deploy(PrizePool.address,
      this.squidBetPlayersRegistration.address)
    ///second round 
    this.squidBetSecondRound = await SquidBetSecondRound.deploy(PrizePool.address,
      this.squidBetStartRound.address)
    /// third round 

    this.squidBetThirdRound = await SquidBetThirdRound.deploy(PrizePool.address,
      this.squidBetSecondRound.address)

    ///// Forth round 

    this.squidBetForthRound = await SquidBetForthRound.deploy(PrizePool.address,
  ...
  it("it does basic checks", async function () {
    ...
    await this.squidBetPlayersRegistration.connect(addr1).registerPlayer({ value: ethers.utils.parseEther("1.0") });
    await this.squidBetPlayersRegistration.connect(addr2).registerPlayer({ value: ethers.utils.parseEther("1.0") });
    await this.squidBetPlayersRegistration.connect(addr3).registerPlayer({ value: ethers.utils.parseEther("1.0") });
    await this.squidBetPlayersRegistration.connect(addr4).registerPlayer({ value: ethers.utils.parseEther("1.0") });
    await this.squidBetPlayersRegistration.connect(addr5).registerPlayer({ value: ethers.utils.parseEther("1.0") });
    ...
    expect(await this.squidBetPlayersRegistration.totalPlayerNumber()).to.equal("5");
    expect(await this.squidBetPlayersRegistration.getRegisterPlayerCount()).to.equal("5");
    expect(await this.squidBetPlayersRegistration.getBalance()).to.equal(ethers.utils.parseEther("4.5"));
    expect(await this.squidBetPlayersRegistration.getIsRegisteredPlayer(addr1.address)).to.equal(true);
    expect(await this.squidBetPlayersRegistration.getIsRegisteredPlayer(addr2.address)).to.equal(true);
    expect(await this.squidBetPlayersRegistration.getIsRegisteredPlayer(addr3.address)).to.equal(true);
    expect(await this.squidBetPlayersRegistration.getIsRegisteredPlayer(addr4.address)).to.equal(true);
    expect(await this.squidBetPlayersRegistration.getIsRegisteredPlayer(addr5.address)).to.equal(true);
  });
  ...
  it("check if registered players are inherited across contract", async function () {
    ...
    await this.squidBetStartRound.connect(addr1).placeBet("1", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetStartRound.connect(addr2).placeBet("1", { value: ethers.utils.parseEther("5.0") });
    ...
    await expect(this.squidBetStartRound.connect(addr1).stopBet()).to.be.revertedWith("Only Admin and Owner can perform this function");


    await this.squidBetStartRound.connect(oracle).reportResult("1", "0")
    await expect(this.squidBetStartRound.connect(addr1).reportResult("1", "0")).to.be.revertedWith("Only Oracle and Owner can perform this function");

    expect(await this.squidBetStartRound.getwinners(addr1.address)).to.equal(true);
    expect(await this.squidBetStartRound.getwinners(addr2.address)).to.equal(true);
    expect(await this.squidBetStartRound.getwinners(addr3.address)).to.equal(true);
    expect(await this.squidBetStartRound.getwinners(addr4.address)).to.equal(true);
    expect(await this.squidBetStartRound.getwinners(addr5.address)).to.equal(false);

    ///second


    await this.squidBetSecondRound.connect(addr1).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetSecondRound.connect(addr2).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetSecondRound.connect(addr3).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetSecondRound.connect(addr4).placeBet("1", { value: ethers.utils.parseEther("5.0") });
    await expect(this.squidBetSecondRound.connect(addr5).placeBet("1", { value: ethers.utils.parseEther("5.0") })).to.be.revertedWith("Not a valid player");
    ...
    expect(await this.squidBetSecondRound.getwinners(addr1.address)).to.equal(true);
    expect(await this.squidBetSecondRound.getwinners(addr2.address)).to.equal(true);
    expect(await this.squidBetSecondRound.getwinners(addr3.address)).to.equal(true);
    expect(await this.squidBetSecondRound.getwinners(addr4.address)).to.equal(false);


    ///////// third round////
    await this.squidBetThirdRound.connect(addr1).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetThirdRound.connect(addr2).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetThirdRound.connect(addr3).placeBet("1", { value: ethers.utils.parseEther("5.0") });
    await expect(this.squidBetThirdRound.connect(addr4).placeBet("1", { value: ethers.utils.parseEther("5.0") })).to.be.revertedWith("Not a valid player");
    ...
    expect(await this.squidBetThirdRound.getwinners(addr1.address)).to.equal(true);
    expect(await this.squidBetThirdRound.getwinners(addr2.address)).to.equal(true);
    expect(await this.squidBetThirdRound.getwinners(addr3.address)).to.equal(false);



    ////Forth Round////

    await this.squidBetForthRound.connect(addr1).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetForthRound.connect(addr2).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await expect(this.squidBetForthRound.connect(addr3).placeBet("1", { value: ethers.utils.parseEther("5.0") })).to.be.revertedWith("Not a valid player");


    await this.squidBetForthRound.connect(Admin).stopBet();
    await this.squidBetForthRound.connect(oracle).reportResult("0", "1");

    expect(await this.squidBetForthRound.getwinners(addr1.address)).to.equal(true);
    expect(await this.squidBetForthRound.getwinners(addr2.address)).to.equal(true);


    ////Final Round ////

    await this.squidBetFinalRound.connect(addr1).placeBet("0", { value: ethers.utils.parseEther("5.0") });
    await this.squidBetFinalRound.connect(addr2).placeBet("0", { value: ethers.utils.parseEther("5.0") });
  ...
  it("SquidBets round cannot place bets twice", async function() {
    ...
    await this.squidBetStartRound.connect(addr1).placeBet("1", { value: ethers.utils.parseEther("5.0") });
    await expect(this.squidBetStartRound.connect(addr1).placeBet("1", { value: ethers.utils.parseEther("5.0") })).to.be.revertedWith("Bet placed already");
  })
  ...
  it("SquidBets final round vote tie", async function() {
    ...
    expect(res).to.eq("Random Draw will decide the only Winner of the Squid Bet Competition");
  ...
  it("SquidBets final round vote side 1 wins", async function() {
    ...
    expect(res).to.eq("Split The Prize Pool equally between every last final Players remaining in the Squid Bet Competition");
  ...
  it("Non-winners can't vote final round", async function() {
    ...
    expect(currVote).to.eq(0);
    await expect(this.squidBetFinalRound.connect(addr1).Vote(1)).to.be.revertedWith("You do not have any winning bet");
  });
  ...
  it("Can't claim cancelled twice", async function() {
    ...
    await expect(this.squidBetStartRound.connect(addr1).claimBetCancelledEvent()).to.be.revertedWith("You do not make any bets")
  ...
  it("Squidbets can't report same winner", async function () {
    ...
    await expect(this.squidBetStartRound.connect(oracle).reportResult("1", "1")).to.be.revertedWith("Winner and loser cannot be the same");
  });
  
  it("Squidbets prize pool withdraw single then withdraw all", async function() {
    await this.squidBetPrizePool.addMultipleWinnersAddress([addr1.address, addr2.address, addr3.address, addr4.address, addr5.address]);
  ...
  it("Squidbets prize pool withdraw all then can't withdraw single", async function() {
    ...
    await expect(this.squidBetPrizePool.connect(addr1).winnerClaimPrizePool()).to.be.revertedWith("Only a Winner of the Squid Bet Competition can claim the Prize Pool");
  });

  it("Squidbets prize pool non-winner can't withdraw", async function() {
    await this.squidBetPrizePool.addMultipleWinnersAddress([addr1.address, addr2.address, addr3.address, addr4.address, addr5.address]);

    await expect(this.squidBetPrizePool.connect(addr6).winnerClaimPrizePool()).to.be.revertedWith("Only a Winner of the Squid Bet Competition can claim the Prize Pool");
  });
