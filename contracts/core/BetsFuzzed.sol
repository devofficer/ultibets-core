...
contract BetsFuzzed is CustomAdmin, ReentrancyGuard {
    ...
    constructor() {
        ultiBetsTreasury = msg.sender;
        transferOwnership(msg.sender);
    }

    // Properties
    function echidna_if_cancelled_then_finished() public view returns (bool) {
        if (isEventCancelled) {
    ...
    ///@param _side the bettor side choice 0(representing YES) and 1(represnting NO).
    ...
    function claimBetCancelledEvent() external nonReentrant {
        ...
        // POST assertions
        assert(contractBalBefore - address(this).balance == BettorBet);
    ...
    function withdrawGain() external nonReentrant {
        ...
        // POST assertions
        assert(contractBalBefore - address(this).balance == gain);
    ...
    function withdrawEarnedFees() external nonReentrant onlyAdmin {
        ...
        // POST assertions
        assert(contractBalBefore - address(this).balance == amount);
    }
