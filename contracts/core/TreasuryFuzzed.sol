...
contract TreasuryFuzzed is ReentrancyGuard {
    ...
    function echidna_withdrawn_cannot_exceed_allocated()
        public
        view
        returns (bool)
    {
        return totalWithdrawn <= totalAllocation;
    }
    ...
    function createAllocation(
        ...
        // POST assertions
        assert(totalWithdrawn <= totalAllocation);
    ...
    function withdraw() external nonReentrant returns (bool hasWithdrew) {
        ...
        uint256 fundingBalBefore = fundingCoin.balanceOf(address(this));
        ...
        // POST assertions
        assert(
            (fundingBalBefore - fundingCoin.balanceOf(address(this))) == amount
        );
        assert(totalWithdrawn <= totalAllocation);
