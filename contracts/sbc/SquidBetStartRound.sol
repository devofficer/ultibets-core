...
interface ISquidBetPlayersRegistration {
    /// interface of squidbets registration contract
    function getIsRegisteredPlayer(address _address)
        external
        view
        returns (bool _registeredPlayers);
}

contract SquidBetStartRound is CustomAdmin, ReentrancyGuard {
    ISquidBetPlayersRegistration private tokenContract;
    enum Side {
        Biden,
        Trump
    }
    ...
    constructor(address _prizePool, ISquidBetPlayersRegistration _tokenContract)
    {
    ...
    function getResult() public view returns (Side _win) {
        _win = result.winner;
    }
    ...
    function placeBet(Side _side) external payable {
        ...
        require(
            tokenContract.getIsRegisteredPlayer(msg.sender) == true, /// this checks if the address has been registered
